...
describe('Footer Component', () => {
  ...
  it('should render footer with logo first and description below for mobile layout', () => {
    const { container } = render(<Footer />);
    const footerItems = container.querySelector('footer > div');
    const logo = screen.getByAltText('PREFEITURA SP LOGO');
    const description = screen.getByText(/Sistema homologado/);

    expect(footerItems).toContainElement(logo);
    expect(footerItems).toContainElement(description);
    expect(footerItems?.children[0]).toBe(logo);
    expect(footerItems?.children[1]).toBe(description);
  });
});
