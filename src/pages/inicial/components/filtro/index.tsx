...
export const FiltroPaginaInicial: React.FC = () => {
  ...
  const obterAreaPromotora = async () => {
    ...
    if (resposta.sucesso) {
      const areaPromotoraId = resposta.dados.filter((item) => item.descricao === perfilSelecionado);
  ...
  useEffect(() => {
    setPerfilSelecionado(perfilSelecionado);
    obterAreaPromotora();
    obterFiltros();
  }, [perfilSelecionado, obterFiltros, perfilAtual]);

  useEffect(() => {
    form.resetFields();
  }, [form, perfilSelecionado]);
