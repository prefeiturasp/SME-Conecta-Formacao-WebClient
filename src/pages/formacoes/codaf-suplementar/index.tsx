...
const CodafSuplementar: React.FC = () => {
  ...
  const [dados, setDados] = useState<CodafSuplementarDTO[]>([]);
  ...
  const onClickEmitirCertificado = async (record: CodafSuplementarDTO) => {
    try {
      setLoading(true);
  ...
  const onClickExportarListaInscritos = async (record: CodafSuplementarDTO) => {
    try {
  ...
  const onClickBaixarRelatorioCodaf = async (record: CodafSuplementarDTO) => {
    try {
      setLoading(true);
  ...
  const getMenuAcoes = (record: CodafSuplementarDTO): MenuProps => {
  ...
  const getCertificadoButtonState = (record: CodafSuplementarDTO, loading: boolean) => {
  ...
  const colunasBase: ColumnsType<CodafSuplementarDTO> = [
    {
      key: 'codigoFormacao',
  ...
  const colunasAdicionais: ColumnsType<CodafSuplementarDTO> = [
    {
      key: 'certificado',
      title: (
        <span>
      ...
      width: 220,
      render: (_: any, record: CodafSuplementarDTO) => {
  ...
  const colunaAcoes: ColumnsType<CodafSuplementarDTO> = [
    {
      key: 'acoes',
      title: 'Ações',
      width: 80,
  ...
  const buscarDados = async (pagina = 1) => {
    ...
    try {
      ...
      const response = await obterCodafSuplementar(filtros);
